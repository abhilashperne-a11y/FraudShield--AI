...
async function initializeUserSession(user) {
    ...
    if (user.language) {
        const langRadio = document.querySelector(`input[name="voice-lang"][value="${user.language}"]`);
...
// --- MODULE 4: VOICE SAFETY ASSISTANT ---
let recognitionInstance = null;
let speechSpeaker = null;

function initVoiceAssistant() {
    const btnMic = document.getElementById("btn-trigger-voice-mic");
    ...
    if (btnMic) {
        btnMic.addEventListener("click", () => {
            ...
            // Check active selected language
            const lang = document.querySelector('input[name="voice-lang"]:checked').value;

            if (btnMic.classList.contains("active-record")) {
                // Stop recognition
                stopSpeechRecognition();
            } else {
                // Start recognition
                if (!SpeechRecognition) {
                    // Fallback since browser has no SpeechRecognition (e.g. desktop browsers, Firefox or private windows)
                    alert("Voice listening is not supported by this browser. Speak command loaded! Use the text input fallback.");
                    simulateVoiceQuery("electricity");
                    return;
                }
                startSpeechRecognition(SpeechRecognition, lang);
            }
        });
    }

    if (btnVoiceSend && voiceTextInput) {
        // Send button trigger
        btnVoiceSend.addEventListener("click", () => {
            const queryText = voiceTextInput.value.trim();
            if (!queryText) return;
            ...
            // Log query
            appendVoiceChatBubble(queryText, "user", "Amit Kumar");
            
            const lang = document.querySelector('input[name="voice-lang"]:checked').value;
            
            // Evaluate locally after a brief scanning delay
            setTimeout(() => {
                interpretVoiceCommand(queryText, lang);
            }, 600);
        });
...
function startSpeechRecognition(RecognitionClass, langCode) {
    const btnMic = document.getElementById("btn-trigger-voice-mic");
    const btnText = document.getElementById("voice-btn-text");
    const recognizedBubble = document.getElementById("voice-recognized-bubble");
    const avatar = document.getElementById("voice-avatar");

    try {
        recognitionInstance = new RecognitionClass();
        recognitionInstance.continuous = false;
        recognitionInstance.interimResults = false;
        
        // Map selected language standard tags
        let locale = "en-US";
        if (langCode === 'hi') locale = "hi-IN";
        if (langCode === 'ml') locale = "ml-IN";
        if (langCode === 'ta') locale = "ta-IN";

        recognitionInstance.lang = locale;

        recognitionInstance.onstart = () => {
            btnMic.classList.add("btn-danger");
            btnMic.classList.add("active-record");
            btnText.innerText = "Listening... Speak Now";
            recognizedBubble.innerText = SpeechData[langCode].listening;
            avatar.classList.add("listening");
        };

        recognitionInstance.onresult = (event) => {
            const transcript = event.results[0][0].transcript;
            recognizedBubble.innerText = `"${transcript}"`;
            
            // Log the user's transcript to chat logs
            appendVoiceChatBubble(transcript, "user", "Amit Kumar");

            // AI interprets transcript locally
            interpretVoiceCommand(transcript, langCode);
        };

        recognitionInstance.onerror = (e) => {
            console.error("Speech recognition error: ", e);
            recognizedBubble.innerText = "Error listening. Reverting to text simulator...";
            stopSpeechRecognition();
        };
...
function stopSpeechRecognition() {
    ...
    if (btnMic) {
        ...
        btnMic.classList.remove("active-record");
...
function interpretVoiceCommand(text, langCode) {
    const lower = text.toLowerCase();
    let responseText = SpeechData[langCode].unrecognized;
    let queryType = "unrecognized";

    if (lower.includes("payment") || lower.includes("safe") || lower.includes("pay") || lower.includes("पैसा") || lower.includes("सुरक्षित") || lower.includes("പണം") || lower.includes("பணம்")) {
        responseText = SpeechData[langCode].safe_response;
        queryType = "safe";
    } else if (lower.includes("lottery") || lower.includes("prize") || lower.includes("win") || lower.includes("लॉटरी") || lower.includes("इनाम") || lower.includes("സമ്മാനം") || lower.includes("பரிசு")) {
        responseText = SpeechData[langCode].lottery_response;
        queryType = "lottery";
    } else if (lower.includes("electricity") || lower.includes("bill") || lower.includes("cut") || lower.includes("बिजली") || lower.includes("കറന്റ്") || lower.includes("மின்சாரம்")) {
        responseText = SpeechData[langCode].electricity_response;
        queryType = "electricity";
    }

    // Trigger vocal readout
    vocalizeResponse(responseText, langCode);
}

function vocalizeResponse(text, langCode) {
    appendVoiceChatBubble(text, "assistant", "FraudShield Voice AI");
    
    // Play waveform visuals
    const waveformBars = document.querySelectorAll(".wave-bar");
    waveformBars.forEach(bar => bar.classList.add("speaking"));
    ...
    if ('speechSynthesis' in window) {
        ...
        // Visual waves clean up on voice end
        utterance.onend = () => {
            waveformBars.forEach(bar => bar.classList.remove("speaking"));
        };
        utterance.onerror = () => {
            waveformBars.forEach(bar => bar.classList.remove("speaking"));
        };
...
function appendVoiceChatBubble(text, senderClass, senderName) {
    const chatLog = document.getElementById("voice-chat-transcript");
    if (chatLog) {
        const bubble = document.createElement("div");
        bubble.className = `chat-bubble ${senderClass}`;
        bubble.innerHTML = `
            <div class="chat-sender font-bold">${senderName}</div>
...
window.simulateVoiceQuery = function(type) {
    AudioSynth.playClick();
    const lang = document.querySelector('input[name="voice-lang"]:checked').value;
    
    let queryText = "Is this transaction safe?";
    let answer = SpeechData[lang].safe_response;

    if (type === 'lottery') {
        queryText = lang === 'en' ? "I received an SMS that I won 5 Lakh, click to claim." : 
                    lang === 'hi' ? "मुझे संदेश मिला है कि मैं 5 लाख जीत गया हूँ।" : 
                    lang === 'ml' ? "എനിക്ക് 5 ലക്ഷം രൂപ സമ്മാനം ലഭിച്ചതായി സന്ദേശം വന്നു." : 
                    "எனக்கு 5 லட்சம் பரிசு கிடைத்துள்ளதாக செய்தி வந்தது.";
        answer = SpeechData[lang].lottery_response;
    } else if (type === 'electricity') {
        queryText = lang === 'en' ? "Will my electricity line disconnect today?" : 
                    lang === 'hi' ? "क्या आज मेरी बिजली काट दी जाएगी?" : 
                    lang === 'ml' ? "ഇന്ന് എന്റെ കറന്റ് കട്ട് ചെയ്യുമോ?" : 
                    "இன்று எனது மின் இணைப்பு துண்டிக்கப்படுமா?";
        answer = SpeechData[lang].electricity_response;
    } else {
        queryText = lang === 'en' ? "Is this payment transaction safe?" : 
                    lang === 'hi' ? "क्या यह भुगतान पूरी तरह सुरक्षित है?" : 
                    lang === 'ml' ? "ഈ പണമിடപാട് സുരക്ഷിതമാണോ?" : 
                    "இந்த கட்டணம் பாதுகாப்பானதா?";
        answer = SpeechData[lang].safe_response;
    }
